...
import org.camel.squadws.SquadWsEndpoint;
...
public class SquadWsServerChannelHandler extends SimpleChannelUpstreamHandler {
	private static final Logger logger = LoggerFactory.getLogger( SquadWsServerChannelHandler.class );
	private final ChannelGroup channelGroup = new DefaultChannelGroup( "squadws-channels" );
	...
	public SquadWsServerChannelHandler( String context ) {
		if ( context == null ) {
			this.context = WEB_SOCKET_CONTEXT;
		}
	}
	...
	private String getWebSocketLocation( HttpRequest request ) {
		return "ws://" + HttpHeaders.getHeader( request, HttpHeaders.Names.HOST ) + "/" + this.context;
	}
